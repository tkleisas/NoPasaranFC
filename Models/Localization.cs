using System;
using System.Collections.Generic;
using System.IO;
using System.Text.Json;

namespace NoPasaranFC.Models
{
    public class Localization
    {
        private static Localization _instance;
        private Dictionary<string, string> _strings;
        private string _currentLanguage;
        
        public static Localization Instance
        {
            get
            {
                if (_instance == null)
                {
                    _instance = new Localization();
                    _instance.LoadLanguage(GameSettings.Instance.Language);
                }
                return _instance;
            }
        }
        
        public void LoadLanguage(string languageCode)
        {
            _currentLanguage = languageCode;
            _strings = GetDefaultStrings(languageCode);
            
            // Try to load from JSON file (optional)
            string filePath = $"Content/Localization/{languageCode}.json";
            if (File.Exists(filePath))
            {
                try
                {
                    string json = File.ReadAllText(filePath);
                    var loaded = JsonSerializer.Deserialize<Dictionary<string, string>>(json);
                    if (loaded != null)
                    {
                        _strings = loaded;
                    }
                }
                catch (Exception ex)
                {
                    System.Diagnostics.Debug.WriteLine($"Failed to load localization file: {ex.Message}");
                }
            }
        }
        
        public string Get(string key)
        {
            if (_strings != null && _strings.ContainsKey(key))
            {
                return _strings[key];
            }
            return key; // Return key if translation not found
        }
        
        private Dictionary<string, string> GetDefaultStrings(string languageCode)
        {
            if (languageCode == "el")
            {
                return GetGreekStrings();
            }
            else
            {
                return GetEnglishStrings();
            }
        }
        
        internal Dictionary<string, string> GetEnglishStrings()
        {
            return new Dictionary<string, string>
            {
                // Menu
                ["menu.standings"] = "VIEW STANDINGS",
                ["menu.nextMatch"] = "NEXT MATCH",
                ["menu.newSeason"] = "NEW SEASON",
                ["menu.settings"] = "SETTINGS",
                ["menu.exit"] = "EXIT",
                ["menu.back"] = "BACK",
                ["menu.seasonComplete"] = "Season Complete!",
                
                // Standings
                ["standings.title"] = "STANDINGS",
                ["standings.pos"] = "POS",
                ["standings.rank"] = "Rank",
                ["standings.team"] = "Team",
                ["standings.wins"] = "W",
                ["standings.draws"] = "D",
                ["standings.losses"] = "L",
                ["standings.gf"] = "GF",
                ["standings.ga"] = "GA",
                ["standings.goalsFor"] = "GF",
                ["standings.goalsAgainst"] = "GA",
                ["standings.points"] = "Pts",
                ["standings.back"] = "Press ESC or B to go back",
                ["standings.toggleHint"] = "TAB: Top Scorers",

                // Match statistics
                ["stats.title"] = "MATCH STATISTICS",
                ["stats.possession"] = "Possession",
                ["stats.shots"] = "Shots",
                ["stats.onTarget"] = "On Target",
                ["stats.passes"] = "Passes",
                ["stats.passAcc"] = "Pass %",
                ["stats.fouls"] = "Fouls",
                ["stats.corners"] = "Corners",
                ["stats.offsides"] = "Offsides",
                ["stats.saves"] = "Saves",
                ["stats.continue"] = "Press ENTER or ESC to continue",
                ["stats.scorers"] = "TOP SCORERS",
                ["stats.goals"] = "G",
                ["stats.assists"] = "A",
                ["stats.cards"] = "Cards",
                ["stats.player"] = "Player",
                ["stats.col.g"] = "G",
                ["stats.col.a"] = "A",
                ["stats.col.sh"] = "SH",
                ["stats.col.ot"] = "OT",
                ["stats.col.ps"] = "PS",
                ["stats.col.tk"] = "TK",
                ["stats.col.fc"] = "FC",
                ["stats.col.yc"] = "YC",
                ["stats.col.rc"] = "RC",
                
                // Match
                ["match.goal"] = "GOAL!",
                ["match.loading"] = "LOADING...",
                ["match.finalScore"] = "FINAL SCORE",
                ["match.halfTime"] = "HALF TIME",
                ["match.throwIn"] = "THROW-IN",
                ["match.cornerKick"] = "CORNER KICK",
                ["match.goalKick"] = "GOAL KICK",
                ["match.power"] = "POWER",
                
                // Round Results
                ["round_results_title"] = "MATCHWEEK {0} RESULTS",
                ["round_results_continue"] = "Press ENTER or SPACE to continue",
                ["round_results_champion"] = "Team {0} is the Champion!",
                
                // Champion award screen
                ["champion.title"] = "CHAMPIONS!",
                ["champion.team_won"] = "{0} won the championship",
                
                // Lineup
                ["lineup.title"] = "SELECT LINEUP",
                ["lineup.starting"] = "STARTING",
                ["lineup.bench"] = "BENCH",
                ["lineup.confirm"] = "Press ENTER to confirm",
                ["lineup.need11"] = "Need 11 starting players!",
                ["lineup.position.gk"] = "GK",
                ["lineup.position.def"] = "DEF",
                ["lineup.position.mid"] = "MID",
                ["lineup.position.fwd"] = "FWD",
                ["lineup.starter"] = "[STARTER]",
                ["lineup.benchPlayer"] = "[BENCH]",
                ["lineup.name"] = "NAME",
                ["lineup.position"] = "POS",
                ["lineup.status"] = "STATUS",
                ["lineup.formationPreview"] = "FORMATION PREVIEW",
                ["lineup.instructions"] = "UP/DOWN: Navigate | SPACE: Toggle Starter | ENTER: Confirm | ESC: Back",
                ["lineup.more"] = "MORE",
                ["lineup.stats"] = "STATS",
                ["lineup.stat.spd"] = "SPD",
                ["lineup.stat.sht"] = "SHT",
                ["lineup.stat.pas"] = "PAS",
                ["lineup.stat.def"] = "DEF",
                ["lineup.stat.agi"] = "AGI",
                ["lineup.stat.tec"] = "TEC",
                ["lineup.position.goalkeeper"] = "Goalkeeper",
                ["lineup.position.defender"] = "Defender",
                ["lineup.position.midfielder"] = "Midfielder",
                ["lineup.position.forward"] = "Forward",
                ["lineup.goalkeepers"] = "GOALKEEPERS",
                ["lineup.defenders"] = "DEFENDERS",
                ["lineup.midfielders"] = "MIDFIELDERS",
                ["lineup.forwards"] = "FORWARDS",
                
                // Settings
                ["settings.title"] = "SETTINGS",
                ["settings.video"] = "=== VIDEO ===",
                ["settings.audio"] = "=== AUDIO ===",
                ["settings.gameplay"] = "=== GAMEPLAY ===",
                ["settings.display"] = "=== DISPLAY ===",
                ["settings.camera"] = "=== CAMERA ===",
                ["settings.language"] = "=== LANGUAGE ===",
                
                ["settings.resolution"] = "Resolution",
                ["settings.fullscreen"] = "Fullscreen",
                ["settings.vsync"] = "VSync",
                ["settings.masterVolume"] = "Master Volume",
                ["settings.musicVolume"] = "Music Volume",
                ["settings.sfxVolume"] = "SFX Volume",
                ["settings.muteAll"] = "Mute All",
                ["settings.difficulty"] = "Difficulty",
                ["settings.matchDuration"] = "Match Duration",
                ["settings.playerSpeed"] = "Player Speed",
                ["settings.showMinimap"] = "Show Minimap",
                ["settings.showNames"] = "Show Player Names",
                ["settings.showStamina"] = "Show Stamina",
                ["settings.cameraZoom"] = "Camera Zoom",
                ["settings.cameraSpeed"] = "Camera Speed",
                ["settings.matchView"] = "Match View",
                ["settings.cameraMode"] = "Camera",
                ["settings.timeOfDay"] = "Time of Day",
                ["settings.weather"] = "Weather",
                ["settings.aiDecisionInterval"] = "AI Decision Rate",
                ["settings.languageSelect"] = "Language",
                
                ["settings.difficulty.easy"] = "Easy",
                ["settings.difficulty.normal"] = "Normal",
                ["settings.difficulty.hard"] = "Hard",
                
                ["settings.cameraMode.broadcast"] = "Broadcast",
                ["settings.cameraMode.high"] = "High",
                ["settings.cameraMode.topdown"] = "Top-Down",
                ["settings.timeOfDay.day"] = "Day",
                ["settings.timeOfDay.sunset"] = "Sunset",
                ["settings.timeOfDay.night"] = "Night",
                ["settings.timeOfDay.random"] = "Random",
                ["settings.weather.clear"] = "Clear",
                ["settings.weather.rain"] = "Rain",
                ["settings.weather.random"] = "Random",
                ["settings.venue"] = "Venue",
                ["settings.venue.bahramis"] = "Bahramis",
                ["settings.venue.sperchogeia"] = "Sperchogeia",
                ["settings.venue.sfageia"] = "Sfageia",
                ["settings.venue.kerasoulia"] = "Kerasoulia",
                ["settings.venue.soulinari"] = "Soulinari",
                ["settings.venue.random"] = "Random",
                ["settings.ballControl"] = "Ball Control",
                ["settings.ballControl.easy"] = "Easy",
                ["settings.ballControl.classic"] = "Classic",
                ["settings.offsides"] = "Offsides",
                ["settings.recordMatches"] = "Match recording",
                ["settings.recordVerbose"] = "Verbose AI decision log",
                ["match.halftime"] = "HALF TIME",
                ["match.offside"] = "OFFSIDE",
                ["match.foul"] = "FOUL",
                ["match.freeKick"] = "FREE KICK",
                ["match.penalty"] = "PENALTY",
                ["match.yellowCard"] = "YELLOW CARD",
                ["match.redCard"] = "RED CARD",
                ["match.secondYellow"] = "second yellow",
                
                ["settings.on"] = "ON",
                ["settings.off"] = "OFF",

                // Championship selection
                ["championship.selectTitle"] = "SELECT CHAMPIONSHIP",
                ["championship.none"] = "No championships available",
                ["championship.teamCount"] = "{0} teams",
                ["championship.instructions"] = "UP/DOWN: Navigate   ENTER: Confirm",
                ["championship.instructionsCancel"] = "UP/DOWN: Navigate   ENTER: Confirm   ESC: Back",

                // Local 2-player mode (desktop only)
                ["match.player2JoinHint"] = "Player 2: RIGHT SHIFT = opposing team, RIGHT ALT = same team (Pad 2: START/BACK)",
                ["match.player2Joined"] = "Player 2 joined",
                ["match.player2Left"] = "Player 2 left",

                // Player/kit editor (desktop only)
                ["menu.editor"] = "EDITOR",
                ["editor.title"] = "EDITOR",
                ["editor.kit"] = "KIT",
                ["editor.back"] = "BACK",
                ["editor.gender"] = "Gender",
                ["editor.skin"] = "Skin Tone",
                ["editor.hair"] = "Hair Color",
                ["editor.expression"] = "Expression",
                ["editor.feature"] = "Facial Feature",
                ["editor.pattern"] = "Pattern",
                ["editor.patternColor"] = "Pattern Color",
                ["editor.shirtColor"] = "Shirt Color",
                ["editor.shortsColor"] = "Shorts Color",
                ["editor.socksColor"] = "Socks Color",
                ["editor.gkShirtColor"] = "GK Shirt Color",
                ["editor.gkShortsColor"] = "GK Shorts Color",
                ["editor.gkSocksColor"] = "GK Socks Color",
                ["editor.paint"] = "PAINT SHIRT",
                ["editor.export"] = "EXPORT",
                ["editor.resetKit"] = "RESET KIT",
                ["editor.exportDone"] = "Exported to:",
                ["editor.auto"] = "Auto",
                ["editor.male"] = "Male",
                ["editor.female"] = "Female",
                ["editor.pattern.solid"] = "Solid",
                ["editor.pattern.stripes"] = "Stripes",
                ["editor.pattern.hoops"] = "Hoops",
                ["editor.pattern.halves"] = "Halves",
                ["editor.pattern.sash"] = "Sash",
                ["editor.expr.smile"] = "Smile",
                ["editor.expr.neutral"] = "Neutral",
                ["editor.expr.sad"] = "Sad",
                ["editor.expr.wow"] = "Wow",
                ["editor.feature.none"] = "None",
                ["editor.feature.beard"] = "Beard",
                ["editor.feature.goatee"] = "Goatee",
                ["editor.feature.sideburns"] = "Sideburns",
                ["editor.feature.eyelashes"] = "Eyelashes",
                ["editor.instructions"] = "UP/DOWN: Navigate   LEFT/RIGHT: Change   ENTER: Select   ESC: Back",
                ["editor.paintInstructions"] = "ARROWS: Move   X: Paint   C: Color   Z/DEL: Erase   ESC: Back",
            };
        }

        internal Dictionary<string, string> GetGreekStrings()
        {
            return new Dictionary<string, string>
            {
                // Menu
                ["menu.standings"] = "ΠΡΟΒΟΛΗ ΑΠΟΤΕΛΕΣΜΑΤΩΝ",
                ["menu.nextMatch"] = "ΕΠΟΜΕΝΟΣ ΑΓΩΝΑΣ",
                ["menu.newSeason"] = "ΝΕΟ ΠΡΩΤΑΘΛΗΜΑ",
                ["menu.settings"] = "ΕΠΙΛΟΓΕΣ",
                ["menu.exit"] = "ΕΞΟΔΟΣ",
                ["menu.back"] = "ΠΙΣΩ",
                ["menu.seasonComplete"] = "Τέλος Σεζόν!",
                
                // Standings
                ["standings.title"] = "ΒΑΘΜΟΛΟΓΙΑ",
                ["standings.pos"] = "ΘΕΣΗ",
                ["standings.rank"] = "Θέση",
                ["standings.team"] = "Ομάδα",
                ["standings.wins"] = "Ν",
                ["standings.draws"] = "Ι",
                ["standings.losses"] = "Η",
                ["standings.gf"] = "ΓΥ",
                ["standings.ga"] = "ΓΚ",
                ["standings.goalsFor"] = "ΓΥ",
                ["standings.goalsAgainst"] = "ΓΚ",
                ["standings.points"] = "Βαθ",
                ["standings.back"] = "Πατήστε ESC ή B για επιστροφή",
                ["standings.toggleHint"] = "TAB: Σκόρερ",

                // Match statistics
                ["stats.title"] = "ΣΤΑΤΙΣΤΙΚΑ ΑΓΩΝΑ",
                ["stats.possession"] = "Κατοχή",
                ["stats.shots"] = "Σουτ",
                ["stats.onTarget"] = "Στο στόχο",
                ["stats.passes"] = "Πάσες",
                ["stats.passAcc"] = "Πάσες %",
                ["stats.fouls"] = "Φάουλ",
                ["stats.corners"] = "Κόρνερ",
                ["stats.offsides"] = "Οφσάιντ",
                ["stats.saves"] = "Αποκρούσεις",
                ["stats.continue"] = "Πατήστε ENTER ή ESC για συνέχεια",
                ["stats.scorers"] = "ΣΚΟΡΕΡ",
                ["stats.goals"] = "Γ",
                ["stats.assists"] = "Α",
                ["stats.cards"] = "Κάρτες",
                ["stats.player"] = "Παίκτης",
                ["stats.col.g"] = "Γ",
                ["stats.col.a"] = "Α",
                ["stats.col.sh"] = "ΣΤ",
                ["stats.col.ot"] = "ΣΣ",
                ["stats.col.ps"] = "ΠΣ",
                ["stats.col.tk"] = "ΤΚ",
                ["stats.col.fc"] = "ΦΑ",
                ["stats.col.yc"] = "ΚΙ",
                ["stats.col.rc"] = "ΚΟ",
                
                // Match
                ["match.goal"] = "ΓΚΟΛ!",
                ["match.loading"] = "ΦΟΡΤΩΣΗ...",
                ["match.finalScore"] = "ΤΕΛΙΚΟ ΣΚΟΡ",
                ["match.halfTime"] = "ΗΜΙΧΡΟΝΟ",
                ["match.throwIn"] = "ΠΛΑΓΙΟ",
                ["match.cornerKick"] = "ΚΟΡΝΕΡ",
                ["match.goalKick"] = "ΕΠΑΝΑΦΟΡΑ",
                ["match.power"] = "ΔΥΝΑΜΗ",
                
                // Lineup
                ["lineup.title"] = "ΕΠΙΛΟΓΗ ΣΥΝΘΕΣΗΣ",
                ["lineup.starting"] = "ΒΑΣΙΚΟΣ",
                ["lineup.bench"] = "ΠΑΓΚΟΣ",
                ["lineup.confirm"] = "Πατήστε ENTER για επιβεβαίωση",
                ["lineup.need11"] = "Χρειάζεστε 11 βασικούς παίκτες!",
                ["lineup.position.gk"] = "ΤΕ",
                ["lineup.position.def"] = "ΑΜ",
                ["lineup.position.mid"] = "ΜΕ",
                ["lineup.position.fwd"] = "ΕΠ",
                ["lineup.starter"] = "[ΒΑΣΙΚΟΣ]",
                ["lineup.benchPlayer"] = "[ΠΑΓΚΟΣ]",
                ["lineup.name"] = "ΟΝΟΜΑ",
                ["lineup.position"] = "ΘΕΣΗ",
                ["lineup.status"] = "ΚΑΤΑΣΤΑΣΗ",
                ["lineup.formationPreview"] = "ΠΡΟΕΠΙΣΚΟΠΗΣΗ ΣΥΝΘΕΣΗΣ",
                ["lineup.instructions"] = "ΑΝΩ/ΚΑΤΩ: Πλοήγηση | SPACE: Εναλλαγή | ENTER: Επιβεβαίωση | ESC: Πίσω",
                ["lineup.more"] = "ΠΕΡΙΣΣΟΤΕΡΑ",
                ["lineup.stats"] = "ΣΤΑΤΙΣΤΙΚΑ",
                ["lineup.stat.spd"] = "ΤΑΧ",
                ["lineup.stat.sht"] = "ΣΟΥ",
                ["lineup.stat.pas"] = "ΠΑΣ",
                ["lineup.stat.def"] = "ΑΜΥ",
                ["lineup.stat.agi"] = "ΕΥΕ",
                ["lineup.stat.tec"] = "ΤΕΧ",
                ["lineup.position.goalkeeper"] = "Τερματοφύλακας",
                ["lineup.position.defender"] = "Αμυντικός",
                ["lineup.position.midfielder"] = "Μέσος",
                ["lineup.position.forward"] = "Επιθετικός",
                ["lineup.goalkeepers"] = "ΤΕΡΜΑΤΟΦΥΛΑΚΕΣ",
                ["lineup.defenders"] = "ΑΜΥΝΤΙΚΟΙ",
                ["lineup.midfielders"] = "ΜΕΣΟΙ",
                ["lineup.forwards"] = "ΕΠΙΘΕΤΙΚΟΙ",
                
                // Round Results
                ["round_results_title"] = "ΑΠΟΤΕΛΕΣΜΑΤΑ {0}ΗΣ ΑΓΩΝΙΣΤΙΚΗΣ",
                ["round_results_continue"] = "Πατήστε ENTER ή SPACE για συνέχεια",
                ["round_results_champion"] = "Η ομάδα {0} είναι Πρωταθλήτρια!",
                
                // Champion award screen
                ["champion.title"] = "ΠΡΩΤΑΘΛΗΤΕΣ!",
                ["champion.team_won"] = "Η ομάδα {0} κατέκτησε το πρωτάθλημα",
                // Settings
                ["settings.title"] = "ΕΠΙΛΟΓΕΣ",
                ["settings.video"] = "=== ΒΙΝΤΕΟ ===",
                ["settings.audio"] = "=== ΗΧΟΣ ===",
                ["settings.gameplay"] = "=== ΠΑΙΧΝΙΔΙ ===",
                ["settings.display"] = "=== ΕΜΦΑΝΙΣΗ ===",
                ["settings.camera"] = "=== ΚΑΜΕΡΑ ===",
                ["settings.language"] = "=== ΓΛΩΣΣΑ ===",
                
                ["settings.resolution"] = "Ανάλυση",
                ["settings.fullscreen"] = "Πλήρης Οθόνη",
                ["settings.vsync"] = "VSync",
                ["settings.masterVolume"] = "Γενική Ένταση",
                ["settings.musicVolume"] = "Ένταση Μουσικής",
                ["settings.sfxVolume"] = "Ένταση Εφέ",
                ["settings.muteAll"] = "Σίγαση Όλων",
                ["settings.difficulty"] = "Δυσκολία",
                ["settings.matchDuration"] = "Διάρκεια Αγώνα",
                ["settings.playerSpeed"] = "Ταχύτητα Παικτών",
                ["settings.showMinimap"] = "Εμφάνιση Μίνι Χάρτη",
                ["settings.showNames"] = "Εμφάνιση Ονομάτων",
                ["settings.showStamina"] = "Εμφάνιση Κόπωσης",
                ["settings.cameraZoom"] = "Ζουμ Κάμερας",
                ["settings.cameraSpeed"] = "Ταχύτητα Κάμερας",
                ["settings.matchView"] = "Προβολή Αγώνα",
                ["settings.cameraMode"] = "Κάμερα",
                ["settings.timeOfDay"] = "Ώρα Ημέρας",
                ["settings.weather"] = "Καιρός",
                ["settings.aiDecisionInterval"] = "Ρυθμός Αποφάσεων ΤΝ",
                ["settings.languageSelect"] = "Γλώσσα",
                
                ["settings.difficulty.easy"] = "Εύκολο",
                ["settings.difficulty.normal"] = "Κανονικό",
                ["settings.difficulty.hard"] = "Δύσκολο",
                
                ["settings.cameraMode.broadcast"] = "Τηλεοπτική",
                ["settings.cameraMode.high"] = "Ψηλή",
                ["settings.cameraMode.topdown"] = "Κάτωθεν",
                ["settings.timeOfDay.day"] = "Μέρα",
                ["settings.timeOfDay.sunset"] = "Δύση",
                ["settings.timeOfDay.night"] = "Νύχτα",
                ["settings.timeOfDay.random"] = "Τυχαία",
                ["settings.weather.clear"] = "Αίθριος",
                ["settings.weather.rain"] = "Βροχή",
                ["settings.weather.random"] = "Τυχαίος",
                ["settings.venue"] = "Γήπεδο",
                ["settings.venue.bahramis"] = "Μπαχράμης",
                ["settings.venue.sperchogeia"] = "Σπερχογεία",
                ["settings.venue.sfageia"] = "Σφαγεία",
                ["settings.venue.kerasoulia"] = "Κερασούλια",
                ["settings.venue.soulinari"] = "Σουληνάρι",
                ["settings.venue.random"] = "Τυχαίο",
                ["settings.ballControl"] = "Έλεγχος Μπάλας",
                ["settings.ballControl.easy"] = "Εύκολος",
                ["settings.ballControl.classic"] = "Κλασικός",
                ["settings.offsides"] = "Οφσάιντ",
                ["settings.recordMatches"] = "Καταγραφή αγώνα",
                ["settings.recordVerbose"] = "Αναλυτική καταγραφή αποφάσεων AI",
                ["match.halftime"] = "ΗΜΙΧΡΟΝΟ",
                ["match.offside"] = "ΟΦΣΑΪΝΤ",
                ["match.foul"] = "ΦΑΟΥΛ",
                ["match.freeKick"] = "ΕΛΕΥΘΕΡΟ",
                ["match.penalty"] = "ΠΕΝΑΛΤΥ",
                ["match.yellowCard"] = "ΚΙΤΡΙΝΗ ΚΑΡΤΑ",
                ["match.redCard"] = "ΚΟΚΚΙΝΗ ΚΑΡΤΑ",
                ["match.secondYellow"] = "δεύτερη κίτρινη",
                
                ["settings.on"] = "ΝΑΙ",
                ["settings.off"] = "ΟΧΙ",

                // Championship selection
                ["championship.selectTitle"] = "ΕΠΙΛΟΓΗ ΠΡΩΤΑΘΛΗΜΑΤΟΣ",
                ["championship.none"] = "Δεν υπάρχουν διαθέσιμα πρωταθλήματα",
                ["championship.teamCount"] = "{0} ομάδες",
                ["championship.instructions"] = "ΑΝΩ/ΚΑΤΩ: Πλοήγηση   ENTER: Επιβεβαίωση",
                ["championship.instructionsCancel"] = "ΑΝΩ/ΚΑΤΩ: Πλοήγηση   ENTER: Επιβεβαίωση   ESC: Πίσω",

                // Τοπική λειτουργία 2 παικτών (μόνο σε υπολογιστή)
                ["match.player2JoinHint"] = "Παίκτης 2: RIGHT SHIFT = αντίπαλη ομάδα, RIGHT ALT = ίδια ομάδα (Χειρ. 2: START/BACK)",
                ["match.player2Joined"] = "Ο Παίκτης 2 συμμετέχει",
                ["match.player2Left"] = "Ο Παίκτης 2 αποχώρησε",

                // Επεξεργαστής παικτών/εμφανίσεων (μόνο σε υπολογιστή)
                ["menu.editor"] = "ΕΠΕΞΕΡΓΑΣΙΑ",
                ["editor.title"] = "ΕΠΕΞΕΡΓΑΣΙΑ",
                ["editor.kit"] = "ΕΜΦΑΝΙΣΗ",
                ["editor.back"] = "ΠΙΣΩ",
                ["editor.gender"] = "Φύλο",
                ["editor.skin"] = "Τόνος Δέρματος",
                ["editor.hair"] = "Χρώμα Μαλλιών",
                ["editor.expression"] = "Έκφραση",
                ["editor.feature"] = "Χαρακτηριστικό",
                ["editor.pattern"] = "Σχέδιο",
                ["editor.patternColor"] = "Χρώμα Σχεδίου",
                ["editor.shirtColor"] = "Χρώμα Φανέλας",
                ["editor.shortsColor"] = "Χρώμα Σορτς",
                ["editor.socksColor"] = "Χρώμα Κάλτσες",
                ["editor.gkShirtColor"] = "Φανέλα Τερμ.",
                ["editor.gkShortsColor"] = "Σορτς Τερμ.",
                ["editor.gkSocksColor"] = "Κάλτσες Τερμ.",
                ["editor.paint"] = "ΖΩΓΡΑΦΙΣΕ ΦΑΝΕΛΑ",
                ["editor.export"] = "ΕΞΑΓΩΓΗ",
                ["editor.resetKit"] = "ΕΠΑΝΑΦΟΡΑ ΕΜΦΑΝΙΣΗΣ",
                ["editor.exportDone"] = "Εξήχθη σε:",
                ["editor.auto"] = "Αυτόματο",
                ["editor.male"] = "Άνδρας",
                ["editor.female"] = "Γυναίκα",
                ["editor.pattern.solid"] = "Μονόχρωμο",
                ["editor.pattern.stripes"] = "Ρίγες",
                ["editor.pattern.hoops"] = "Οριζόντιες",
                ["editor.pattern.halves"] = "Διχρωμία",
                ["editor.pattern.sash"] = "Διαγώνια",
                ["editor.expr.smile"] = "Χαμόγελο",
                ["editor.expr.neutral"] = "Ουδέτερη",
                ["editor.expr.sad"] = "Λύπη",
                ["editor.expr.wow"] = "Έκπληξη",
                ["editor.feature.none"] = "Κανένα",
                ["editor.feature.beard"] = "Γενειάδα",
                ["editor.feature.goatee"] = "Κοντή Γενειάδα",
                ["editor.feature.sideburns"] = "Φαβορίτες",
                ["editor.feature.eyelashes"] = "Βλεφαρίδες",
                ["editor.instructions"] = "ΑΝΩ/ΚΑΤΩ: Πλοήγηση   ΑΡΙΣΤ/ΔΕΞΙΑ: Αλλαγή   ENTER: Επιλογή   ESC: Πίσω",
                ["editor.paintInstructions"] = "ΒΕΛΗ: Κίνηση   X: Χρώμα   C: Αλλαγή Χρώματος   Z/DEL: Σβήσιμο   ESC: Πίσω",
            };
        }
        
        public static void ReloadLanguage()
        {
            if (_instance != null)
            {
                _instance.LoadLanguage(GameSettings.Instance.Language);
            }
        }
    }
}
