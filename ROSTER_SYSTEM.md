...
## Overview
The game supports flexible roster sizes for each team (minimum 11, default 25), with starting lineups (11 players) and substitutes. Teams and players can be seeded from a JSON file or auto-generated.
...
### Seeding Behavior
1. **With JSON file**: If `teams_seed.json` exists, the game loads teams from it (any roster size)
2. **Auto-generation**: If a team has empty `players` array, 25 players are auto-generated by default with:
   - 2 Goalkeepers (1 starting)
   - 8 Defenders (4 starting)
   - 10 Midfielders (4 starting)
   - 5 Forwards (2 starting)
   - Position distribution scales automatically with roster size
...
- Sequential shirt numbers (1, 2, 3, ...)
- Greek names (Κώστας, Γιώργος, Δημήτρης, etc.)
- Roster size is configurable (default: 25 players)
...
### ✅ Implemented
- [x] Flexible roster sizes (minimum 11, no maximum)
...
The default `teams_seed.json` can include a fully defined NO PASARAN! roster with:
- Any number of players (e.g., 22-25)
...
2. Game will generate 25 players automatically (configurable in code)
3. First 11 will be marked as starting
4. Stats will be position-appropriate
5. Position distribution scales with roster size
